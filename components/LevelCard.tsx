...
export function LevelCard({ level }: LevelCardProps) {
  const content = (
    <div
      className={[
        "relative h-full overflow-hidden rounded-3xl border p-4 transition sm:p-5",
        level.available
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-xl font-bold text-white sm:text-2xl">{level.title}</h2>
          <p className="mt-1.5 text-xs leading-relaxed text-slate-300 sm:text-sm">
            {level.description}
          </p>
        </div>
        {!level.available && (
          <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-slate-800/80 px-2 py-1 text-[10px] font-medium text-slate-300 sm:text-xs">
